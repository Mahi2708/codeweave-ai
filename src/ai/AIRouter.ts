...
export class AIRouter {
    ...
    async configure(): Promise<void> {
        ...
        if (key !== undefined) {
            ...
            vscode.window.showInformationMessage(
                `${provider} API key saved securely in VS Code SecretStorage.`
            );
        }
    }
    ...
    async generate(
        ...
        if (!apiKey) {
            ...
            throw new Error(
                `No ${providerId} API key is configured. Run "Editree: Configure AI Provider".`
            );
        }
