...
export function activate(context: vscode.ExtensionContext): void {
    ...
    const tree = await vscode.window.withProgress(
      {
        ...
        title: "Editree: Scanning project...",
      },
      () =>
        scanner.scan(root, {
          respectGitignore: cfg.get<boolean>("respectGitignore", true),
          useFrameworkIgnores: cfg.get<boolean>(
            "useFrameworkIgnores",
            true
          ),
          showIgnored: cfg.get<boolean>(
            "showIgnoredInSelector",
            false
          ),
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      "codeweave.createFile",
      async (resource?: vscode.Uri) => {
        const parent = await resolveDirectory(resource);
        if (!parent) return;

        const name = await vscode.window.showInputBox({
          prompt: "File name, e.g. src/example.ts",
        });
    ...
    vscode.commands.registerCommand(
      ...
      async (resource?: vscode.Uri) => {
        ...
        if (!uri) {
          vscode.window.showWarningMessage(
            "Open or select a source file first."
          );
          return;
        }
        ...
        const targetLanguage = await vscode.window.showInputBox({
          ...
          placeHolder:
            "e.g. Python, TypeScript + React, Kotlin, Spring Boot",
        });

        if (!targetLanguage) return;

        const instruction = await vscode.window.showInputBox({
          prompt: "What should Editree do?",
        ...
        const clipped =
          source.length > maxChars
            ? source.slice(0, maxChars) +
              "\n/* Context truncated by Editree. */"
            : source;

        if (
          cfg.get<boolean>("confirmAiFileSend", true)
        ) {
          const confirm =
            await vscode.window.showWarningMessage(
              `Editree will send ${Math.min(
                source.length,
                maxChars
              ).toLocaleString()} characters to your configured AI provider.`,
              { modal: true },
              "Continue"
            );
        ...
        const prompt = [
          "You are Editree AI, a careful software transformation assistant.",
          instruction,
        ...
        const targetName = await vscode.window.showInputBox({
          prompt:
            "Output file name (leave blank to show result in a new editor)",
          value: "",
        });
    ...
    vscode.commands.registerCommand(
      ...
      async (resource?: vscode.Uri) => {
        ...
        const request = await vscode.window.showInputBox({
          ...
          placeHolder:
            "Create a REST controller for User CRUD...",
        });

        if (!request) return;

        const cfg =
          vscode.workspace.getConfiguration("codeweave");
        ...
        if (
          ...
          const confirm =
            await vscode.window.showWarningMessage(
              "Editree will send your generation prompt to the configured AI provider.",
              { modal: true },
              "Continue"
            );
